...
$ModifyDoctors = $doctor->modifyDoctor($SpecificDoctors[0]['doctors_name'], $SpecificDoctors[0]['doctors_lastname'], $SpecificDoctors[0]['doctors_phonenumber'], $SpecificDoctors[0]['doctors_mail'], $SpecificDoctors[0]['medicalspecialities_id_medicalspecialities'], $SpecificDoctors[0]['doctors_id']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (count($errors) == 0) {
        ...
        // $userObj = new Users();
        // $newUser = $userObj->addUsers($doctorMail, $users_password, $role_id_role);
        // $AllUsers = $userObj->getAllUsers(); 
        // $modifyUser = $userObj->modifyUser($doctorMail, $users_password, $role_id_role, $AllUsers[0]['users_id']); 

        $doctor = new Doctors();
        $AllDoctorsArray = $doctor->getAllDoctors();
        $SpecificDoctors = $doctor->getSpecificDoctor($_GET['doctors']);
        $ModifyDoctors = $doctor->modifyDoctor($doctorName, $doctorSurname, $doctorPhone, $doctorMail, $medicalspecialities_id_medicalspecialities, $SpecificDoctors[0]['doctors_id']);
